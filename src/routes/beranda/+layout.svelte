...
<div
	class:hideSidebarDekstop={!$sidebarOpened}
	class="hidden md:grid lg:grid xl:grid 2xl:grid place-items-center w-full h-full absolute z-50"
>
	<div
		class="bg-gradient-to-b from-biru to-blue-400 p-5 rounded-xl text-white grid grid-cols-1 gap-2"
	>
		<div class="flex gap-3 justify-center items-center mb-3">
			{#if data.user.jenis_kelamin == 'laki laki'}
				<img src="/undraw/profil-pria.svg" alt="" class="bg-white rounded-full w-16" />
			{:else}
				<img src="/undraw/profil-wanita.svg" alt="" class="bg-white rounded-full w-16" />
			{/if}
			<div>
				<h1 class="text-lg font-semibold">{data.user.nama}</h1>
		...
		<div class="flex justify-center mt-3">
			<button
				class="bg-blue-300 p-2 w-fit h-fit rounded-full"
...
<div
	...
	class="block md:hidden lg:hidden xl:hidden 2xl:hidden w-full z-50 h-screen transition-all duration-300 fixed px-4 py-7 right-0 top-0 bg-gradient-to-b from-biru to-blue-400 text-white"
>
